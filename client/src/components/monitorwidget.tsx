...
export default function MonitorWidget({
  ...
  return (
    ...
    >
      ...
      {isRobot ? (
        (() => {
          const eVal = Math.min(Math.max(typeof propEnergy === "number" ? propEnergy : 0.75, 0), 1);
          const w = size * 0.6;
          const h = size * 0.6;
          const x = size / 2 - w / 2;
          const y = size / 2 - h / 2;
          ...
          return (
            <g>
              ...
              <image href="/zap-fill.png" x={x} y={y} width={w} height={h} style={{ clipPath: `inset(${insetTop}% 0 0 0)` }} />
            </g>
          );
        })()
      ) : (
